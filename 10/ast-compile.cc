...
Code_For_Ast & Conditional_Expression_Ast::compile(){
	...
	stmt=new Control_Flow_IC_Stmt(beq, copd,lab1);
	cca->append_ics(*stmt);
...
Code_For_Ast & Relational_Expr_Ast::compile(){
	...
	if (rhs_condition->get_data_type()==int_data_type){
		...
		if (rel_op==greater_than || rel_op==greater_equalto || rel_op==not_equalto)
			stmt=new Control_Flow_IC_Stmt(bc1f, NULL, lab1);
		else 
			stmt=new Control_Flow_IC_Stmt(bc1t, NULL, lab1);
		lca->append_ics(*stmt);
		//lab2 for rest
...
Code_For_Ast & Iteration_Statement_Ast::compile(){
	...
	//
	string lab1=get_new_label(), lab2=get_new_label();
	stmt=new Control_Flow_IC_Stmt(bne, copd,lab1);
	cca->append_ics(*stmt);
	//
	if (! is_do_form){
